...
This package contains some utilities used by the various testing scripts.

The major method of the object is the static subroutine L</Display>. However,
that method requires a fancy data structure to manage a symbol map of things
found. Therefore, this package is also an object package for that data
structure.
...
is currently residing in B<default.js>.
...
the type will be C<ARRAY> or C<HASH>. Otherwise, the type will be undefined.
This method is used to determine how to treat a value when producing a table or
matrix.
...
Returns C<ARRAY> if the value can be treated as an array reference, C<HASH> if
the value can be treated as a hash reference, and an undefined value otherwise.
...
sub DisplayThing {
    ...
    if (! defined $value) {
        ...
        # Pure spaces. These need to be converted to something un-spacelike or
